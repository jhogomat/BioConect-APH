...
export const APHForm: React.FC<APHFormProps> = ({ onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Reset fields
    setFormData(prev => ({ ...prev, patientName: '', patientAge: 0, triggerId: '', observations: '', painScale: 0, location: '', temperature: '', glucose: '', mainComplaint: '', healthInsurance: '', collectedCI: false, numberOfTriggers: 1 }));
    ...
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 space-y-10 relative">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Classificação do Atendimento</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Hora Acionamento</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Hora Chegada Local</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Nº Acionamentos</label>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-2">
            ...
            <input type="text" required className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none" value={formData.patientName} onChange={e => setFormData({...formData, patientName: e.target.value})} />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Idade</label>
            <input type="number" required className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none" value={formData.patientAge} onChange={e => setFormData({...formData, patientAge: parseInt(e.target.value) || 0})} />
          </div>
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={captureGPS} disabled={locating} className="p-2.5 bg-slate-900 text-white rounded-lg hover:bg-slate-700">
      ...
      <div>
        ...
        <div className="space-y-4 bg-slate-50 p-6 rounded-xl border border-slate-200">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <span className="text-sm font-bold text-blue-800 w-32">D (Neurológico):</span>
      ...
      <div>
        <SectionTitle>5. GERENCIAMENTO DE RISCOS ASSISTENCIAIS (CONCEITOS OF QUALIDADE)</SectionTitle>
        <div className="space-y-6 bg-slate-50 p-6 rounded-xl border border-slate-200">
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-4">
            ...
            {riskManagement.identificationChecked && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pl-8 animate-in slide-in-from-left duration-200">
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase">Confirmada com:</label><div className="flex gap-4 mt-1">{['Paciente', 'Familiar'].map(v => (<label key={v} className="flex items-center gap-2 text-xs text-slate-600"><input type="radio" name="confirmedWith" value={v} checked={riskManagement.confirmedWith === v} onChange={e => setRiskManagement({...riskManagement, confirmedWith: e.target.value as any})} /> {v}</label>))}</div></div>
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase">Nome</label><input type="text" className="w-full p-2 bg-white border border-slate-200 rounded text-xs mt-1" value={riskManagement.confirmedName} onChange={e => setRiskManagement({...riskManagement, confirmedName: e.target.value})} /></div>
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase">Data de Nascimento</label><input type="date" className="w-full p-2 bg-white border border-slate-200 rounded text-xs mt-1" value={riskManagement.confirmedBirthDate} onChange={e => setRiskManagement({...riskManagement, confirmedBirthDate: e.target.value})} /></div>
              </div>
            )}
          </div>
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-700">Risco de Queda (Protocolo MS)</span>
            </div>
            {riskManagement.fallRiskActive && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pl-8 animate-in slide-in-from-left duration-200">
                {['Grades elevadas', 'Cinto de segurança', 'Pulseira de risco', 'Calçados adequados', 'Orientação paciente/familiar', 'Leito travado'].map(opt => (<ChoiceButton key={opt} label={opt} selected={riskManagement.fallPreventions.includes(opt)} onClick={() => toggleRiskList('fallPreventions', opt)} />))}
              </div>
            )}
          </div>
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-4">
            <div className="flex items-center gap-3">
              <input type="checkbox" className="w-5 h-5 accent-blue-600" checked={riskManagement.bronchoaspirationRiskActive} onChange={e => setRiskManagement({...riskManagement, bronchoaspirationRiskActive: e.target.checked})} />
              <span className="text-sm font-bold text-slate-700">Risco de Broncoaspiração</span>
            </div>
            {riskManagement.bronchoaspirationRiskActive && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pl-8 animate-in slide-in-from-left duration-200">
                {['Cabeceira elevada > 45°', 'Higiene oral 3x/dia', 'Pausar dieta em transporte', 'Lateralização da cabeça', 'Aspiração de vias aéreas', 'Dieta espessada/macia'].map(opt => (<ChoiceButton key={opt} label={opt} selected={riskManagement.bronchoaspirationPreventions.includes(opt)} onClick={() => toggleRiskList('bronchoaspirationPreventions', opt)} />))}
              </div>
            )}
          </div>
          <div className="flex flex-col gap-2">
            ...
            {riskManagement.medicationSafetyActive && (
              <div className="grid grid-cols-3 md:grid-cols-5 gap-2 pl-8 animate-in slide-in-from-left duration-200">
                {['Paciente certo', 'Med. certo', 'Via certa', 'Hora certa', 'Dose certa', 'Registro certo', 'Orientação certa', 'Forma certa', 'Resposta certa'].map(opt => (<ChoiceButton key={opt} label={opt} selected={riskManagement.medicationSafetyChecks.includes(opt)} onClick={() => toggleRiskList('medicationSafetyChecks', opt)} />))}
              </div>
            )}
          </div>
        </div>
      ...
      {/* SECTION 6 - NURSING IMPLEMENTATION */}
      <div>
        <SectionTitle>6. IMPLEMENTAÇÃO DA ASSISTÊNCIA (ENFERMAGEM)</SectionTitle>
        <div className="space-y-6 bg-slate-50 p-6 rounded-xl border border-slate-200">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">Procedimentos</label>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              {['Acesso Venoso', 'Oxigênio (O2)', 'Imobilização', 'Curativo', 'Outros'].map(proc => (
                <ChoiceButton 
                  key={proc} 
                  label={proc} 
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-3">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Medicamentos/Soluções</label>
              <textarea 
                rows={2}
                className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500" 
                value={nursingImplementation.medicationsSolutions}
                onChange={e => setNursingImplementation({...nursingImplementation, medicationsSolutions: e.target.value})}
                placeholder="Liste os medicamentos e soluções administradas..."
              />
            </div>
            <div>
              ...
              <input 
                type="time"
                className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500"
                value={nursingImplementation.medicationTime}
                onChange={e => setNursingImplementation({...nursingImplementation, medicationTime: e.target.value})}
              />
            </div>
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Evolução/Resposta</label>
            <textarea 
              rows={3}
              className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500" 
              ...
              placeholder="Descreva a evolução do paciente e resposta aos procedimentos..."
            />
          </div>
        </div>
      </div>

      {/* SECTION 7 - SBAR OUTCOME */}
      <div>
        ...
        <div className="space-y-6 bg-slate-50 p-6 rounded-xl border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Hospital de Destino</label>
              <textarea 
                rows={2}
                className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500" 
                value={sbarOutcome.destinationHospital}
                onChange={e => setSbarOutcome({...sbarOutcome, destinationHospital: e.target.value})}
                placeholder="Identifique a unidade de saúde de destino..."
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Passagem de Plantão</label>
              <input 
                type="text"
                className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500" 
                value={sbarOutcome.handoverReport}
                onChange={e => setSbarOutcome({...sbarOutcome, handoverReport: e.target.value})}
                placeholder="Relato sucinto da passagem..."
              />
            </div>
          </div>
          ...
          <div className="border-t border-slate-200 pt-6">
            <p className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
              <i className="fas fa-users text-blue-500"></i> Equipe Responsável pelo Atendimento
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="bg-white p-3 rounded-lg border border-slate-200">
                ...
                <input 
                  type="text"
                  className="w-full p-1 border-b border-slate-100 outline-none text-xs font-medium" 
                  value={team.driverName}
                  onChange={e => setTeam({...team, driverName: e.target.value})}
              ...
              <div className="bg-white p-3 rounded-lg border border-slate-200">
                ...
                <input 
                  type="text"
                  className="w-full p-1 border-b border-slate-100 outline-none text-xs font-medium mb-1" 
                  value={team.nurseName}
                  onChange={e => setTeam({...team, nurseName: e.target.value})}
                  placeholder="Nome do enfermeiro"
                />
                <input 
                  type="text"
                  className="w-full p-1 outline-none text-[10px]" 
                  value={team.nurseCoren}
                  onChange={e => setTeam({...team, nurseCoren: e.target.value})}
                  placeholder="COREN"
                />
              </div>
              <div className="bg-white p-3 rounded-lg border border-slate-200">
                ...
                <input 
                  type="text"
                  className="w-full p-1 border-b border-slate-100 outline-none text-xs font-medium mb-1" 
                  value={team.doctorName}
                  onChange={e => setTeam({...team, doctorName: e.target.value})}
                  placeholder="Nome do médico"
                />
                <input 
                  type="text"
                  className="w-full p-1 outline-none text-[10px]" 
                  value={team.doctorCrm}
                  onChange={e => setTeam({...team, doctorCrm: e.target.value})}
                  placeholder="CRM"
                />
              </div>
            </div>
      ...
      <div className="pt-6">
        <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 px-6 rounded-xl shadow-xl shadow-blue-100 transition-all flex items-center justify-center gap-3 text-lg uppercase tracking-tight">
          <i className="fas fa-check-double"></i> Concluir e Enviar Ficha
        </button>
      </div>
    </form>
  );
